<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 092 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 092 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。

<!-- xxd-human-intro:start -->
## 适用场景与解决的问题

整洁矢量线会抹掉手势，写实描摹又会淹没负形与版式；092 用松散轮廓和多种排线，在删减之后重新组织照片的图形关系。

**Panel 092** 用松散而准确的 contour line drawing 保留主体身份，以 diagonal hatching、cross-hatching 和 scribble hatching 塑造暗部与结构转折。自由曲线负责人物、动物、植物与器物，成组斜线和线性几何负责桌面、墙面、地面、家具与阴影秩序，让松弛与稳定骨架彼此牵制。

### 适合这些情况

- **想要强烈黑白图形感：** 让密集排线的深色量块与大面积负形共同建立主次。
...
> 用 XXD Panel 092 帮我处理这张图，先推荐最合适的构图和尺寸。
...
**关键词：** 表现性钢笔 · 自由轮廓 · 几何排线 · 潦草排线 · 负形构图 · 黑白密度

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 092 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一幅兼具观察速度、黑白图形感和编辑版式的表现性钢笔插画 |
| **一眼可辨的特点** | 松散轮廓、斜向排线、交叉排线、scribble hatching、深色量块与大片负形 |
...
```text
分析主体、轮廓、结构、姿态与叙事关系 → 只保留关键身份和场景逻辑 → 用松散而准确的自由轮廓建立形体 → 以斜向、交叉与潦草排线组织暗部和结构转折 → 用自由曲线表现有机主体、几何排线稳定环境骨架 → 以深色密度块对比大面积负形 → 加入少量克制编辑文字
```

## 成品中最容易识别的风格特点

- 画面以黑、灰、纸白为主，可有极少量低饱和辅助色，但钢笔线描和黑白密度始终是核心。
- 暗部与结构转折由 diagonal、cross 和 scribble hatching 建立，密度可以不完全均匀。
- 复杂背景只保留必要的桌面、地面、边界或空间线索，让留白与排线节奏共同呼吸。
- 文字小、轻、克制，沿留白、边界或线性结构自然分布，不形成标题栏或菜单信息。
- 拒绝写实插画、粗黑漫画线、整洁矢量线、卡通、过度细节、电脑滤镜和模板化海报。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-092-prompt.en.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 092 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-092 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-092 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-092 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 092；公共设置只解析一次
/xxd-panel-092 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-092`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-092-prompt.en.md) · [中文运行适配器](references/xxd-panel-092-prompt.zh-CN.md)
...
| **[xxd-panel-092](https://github.com/nevertoday/xxd-panel-092)** | 表现性钢笔 · 自由轮廓 · 几何排线 · 潦草排线 · 负形构图 · 黑白密度 |
...
| **士兵级** | `xxd-panel-NNN`（当前 001–092） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
