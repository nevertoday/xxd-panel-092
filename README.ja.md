<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 092 プロジェクトバナー" width="1200"></p>
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 092 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

整然としたベクター線は身振りを消し、写実的な描き写しは負形とレイアウトを埋没させます。092 は意図的に要素を削ぎ落としたうえで、ゆるい輪郭と複数の排線によって写真のグラフィックな関係を再編成します。

**Panel 092** は、ゆるやかでありながら正確な contour line drawing で主体の同一性を保ち、diagonal hatching、cross-hatching、scribble hatching で暗部と構造の転換を形作ります。人物、動物、植物、器物は自由な曲線で、卓上、壁、地面、家具、影の方向はまとまりのある斜線と線形幾何で組み、ゆるさと安定した骨格を拮抗させます。

### 向いている用途

- 密な排線の暗い量塊と大きな負形で、強い白黒のグラフィック性と明確な主従を作りたい場合
- 素早く自由で少し走り書きのような線、重ねた修正、均一でない密度による観察感と速度感が必要な場合
- 有機的な主体を自由輪郭、環境の骨格を幾何排線が担う、ゆるさと秩序の緊張を求める場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 092 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 表現的ペン画 · 自由輪郭 · 幾何排線 · スクリブル排線 · 負形構成 · 白黒密度
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 観察の速度、白黒のグラフィック性、編集的レイアウトを兼ね備えた表現的なペン線画 |
| **識別しやすい特徴** | ゆるい輪郭、斜線排線、交差排線、scribble hatching、暗い量塊、大きな負形 |
| **入力の尊重** | 重要な同一性と場面の論理を保ち、複雑な背景を積極的に削ぎ落として、位置・密度・リズムを再編成する |
...
```text
主体・輪郭・構造・ポーズ・物語上の関係を分析 → 重要な同一性と場面の論理だけを残す → ゆるやかで正確な自由輪郭で形を築く → 斜線・交差・スクリブル排線で暗部と構造の転換を組む → 有機的主体は自由曲線、環境骨格は幾何排線で表す → 暗い密度の量塊と大きな負形を対比する → 少量の抑制的な編集文字を加える
```

## 完成作の特徴

- 黒、グレー、紙の白を中心に、ごく少量の低彩度補助色は許容するが、ペン線画と白黒密度を常に核にする。
- 暗部と構造の転換は diagonal hatching、cross-hatching、scribble hatching で作り、密度は完全に均一でなくてよい。
- 複雑な背景は必要な卓上、地面、境界、空間の手掛かりだけを残し、余白と排線のリズムをともに呼吸させる。
- 文字は小さく、軽く、抑制的にし、余白、境界、線形構造に沿って自然に置き、タイトルバーやメニュー情報にしない。
- 写実的なイラスト、太い黒の漫画線、整然としたベクター線、カートゥーン、過剰な細部、コンピューターフィルター感、テンプレート化されたポスター感を避ける。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-092-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 092 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-092 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-092 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-092 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-092 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 092 を適用し、共通設定は一度だけ解決
/xxd-panel-092 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-092`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-092-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-092-prompt.zh-CN.md)
...
| **[xxd-panel-092](https://github.com/nevertoday/xxd-panel-092)** | 表現的ペン画 · 自由輪郭 · 幾何排線 · スクリブル排線 · 負形構成 · 白黒密度 |
...
| **兵士級** | `xxd-panel-NNN`（現在 001–092） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
