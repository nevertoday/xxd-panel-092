<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 092 프로젝트 배너" width="1200"></p>
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 092이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

정돈된 벡터선은 제스처를 지우고 사실적인 묘사는 네거티브 스페이스와 레이아웃을 묻어 버립니다. 092는 의도적으로 요소를 덜어 낸 뒤 느슨한 윤곽과 여러 해칭으로 사진의 그래픽 관계를 다시 구성합니다.

**Panel 092**은 느슨하면서도 정확한 contour line drawing으로 피사체의 정체성을 보존하고 diagonal hatching, cross-hatching, scribble hatching으로 어두운 부분과 구조의 전환을 만듭니다. 사람, 동물, 식물, 사물은 자유로운 곡선으로, 테이블·벽·바닥·가구·그림자 방향은 묶음 사선과 선형 기하로 조직해 느슨함과 안정된 골격을 긴장시킵니다.

### 이런 경우에 적합합니다

- 촘촘한 해칭의 어두운 덩어리와 넓은 네거티브 스페이스로 강한 흑백 그래픽과 명확한 위계를 만들고 싶을 때
- 빠르고 자유로우며 조금 거친 선, 반복 수정, 고르지 않은 밀도에서 관찰감과 속도감을 얻고 싶을 때
- 유기적 피사체는 자유 윤곽이, 환경 골격은 기하 해칭이 맡는 느슨함과 질서의 긴장을 원할 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 092로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 표현적 펜화 · 자유 윤곽 · 기하 해칭 · 스크리블 해칭 · 네거티브 스페이스 · 흑백 밀도
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 관찰의 속도, 강한 흑백 그래픽, 편집적 레이아웃을 함께 갖춘 표현적 펜 선화 |
| **대표 특징** | 느슨한 윤곽, 사선 해칭, 교차 해칭, scribble hatching, 어두운 덩어리, 넓은 네거티브 스페이스 |
| **입력 존중 방식** | 핵심 정체성과 장면 논리를 보존하고 복잡한 배경을 적극적으로 덜어 낸 뒤 위치·밀도·리듬을 재구성함 |
...
```text
피사체·윤곽·구조·자세·서사 관계 분석 → 핵심 정체성과 장면 논리만 보존 → 느슨하면서도 정확한 자유 윤곽으로 형태 구축 → 사선·교차·스크리블 해칭으로 어두운 부분과 구조 전환 조직 → 유기적 피사체는 자유 곡선, 환경 골격은 기하 해칭으로 표현 → 어두운 밀도 덩어리와 넓은 네거티브 스페이스 대비 → 소량의 절제된 편집 문구 추가
```

## 완성작의 특징

- 검정, 회색, 종이의 흰색을 중심으로 하고 저채도 보조색은 극소량 허용하되, 펜 선화와 흑백 밀도를 항상 핵심으로 둡니다.
- 어두운 부분과 구조 전환은 diagonal hatching, cross-hatching, scribble hatching으로 만들며 밀도는 완전히 균일하지 않아도 됩니다.
- 복잡한 배경은 필요한 테이블 면, 지면, 경계, 공간 단서만 남겨 여백과 해칭 리듬이 함께 호흡하게 합니다.
- 글자는 작고 가볍고 절제되게 여백, 경계, 선형 구조를 따라 자연스럽게 배치하며 제목 표시줄이나 메뉴 정보로 만들지 않습니다.
- 사실적인 일러스트, 굵은 검정 만화선, 정돈된 벡터선, 만화적 느낌, 과도한 세부, 컴퓨터 필터 효과, 템플릿형 포스터 미학을 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-092-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 092의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-092 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-092 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-092 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-092 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-092 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-092 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 092 적용, 공통 설정은 한 번만 확정
/xxd-panel-092 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-092`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-092-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-092-prompt.zh-CN.md)
...
| **[xxd-panel-092](https://github.com/nevertoday/xxd-panel-092)** | 표현적 펜화 · 자유 윤곽 · 기하 해칭 · 스크리블 해칭 · 네거티브 스페이스 · 흑백 밀도 |
...
| **병사급** | `xxd-panel-NNN` (현재 001–092) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
