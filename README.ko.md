...
예시 영역은 예약되어 있습니다. 검증 가능한 원작 링크나 092 원본 프롬프트와 직접 대조한 확장 예시가 준비되면 이곳에 먼저 표시하며, 다른 번호의 이미지나 깨진 자리표시자로 대신하지 않습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

사진 외곽선 필터는 선만 복제하고, 지나치게 완성된 사실 스케치는 멈춤과 여백의 절제를 잃습니다.

**Panel 092**은 정확한 윤곽으로 정체성을 남기고 한 가지 파란 연필이나 볼펜의 방향 해칭, 교차 해칭, 탐색선, 필압, 마른 입자로 명암·부피·재질을 엮습니다. 식별 부위만 세밀하게 하고 나머지는 의도적으로 미완성에 둡니다.

### 이런 경우에 적합합니다

- 사진 뒤의 사고 과정, 관계, 방향과 감정을 표현하고 싶을 때
- 삼각 구조, 꺾은선, 교차축, 방사선, 등고선과 장의 궤적을 예술적으로 사용하고 싶을 때
- 날카로운 이성적 기하와 반복 획·눈금·기호·점·손의 어긋남 사이의 긴장을 원할 때
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 독립 여행 스케치북이나 문화 잡지 같은 단색 파란 펜 서사 선화 |
| **대표 특징** | 파란 윤곽, 방향 해칭, 교차 해칭, 탐색선, 선택적 세부, 넓은 자연 종이 |
...
```text
주제·윤곽·자세·관계 분석 → 개념 중심 설정 → 동작·방향·공간·감정·관계·은유 분해 → 글자·노드·궤적·영역·화살표로 의미 체계 구축 → 삼각·꺾은선·교차축·방사선 구성 → 등고선·장선·파형·방향 마크 추가 → 넓은 여백으로 시각 기보 완성
```

## 완성작의 특징

- 기하선은 선명하고 날카롭고 절제되며, 손의 흔적은 가벼운 어긋남·단절·불균일을 허용합니다.
- 검정·목탄회색이 글자와 골격을 맡고 원본의 1~3색이 노드·궤적·밀도장을 맡습니다.
- 글자는 읽히면서도 회전·반복·겹침·경로 배열·부분 분해로 좌표와 시각 기보가 됩니다.
- 넓은 밝은 여백을 보존하고 규칙 격자로 화면을 채우지 않습니다.
- 전통 마인드맵, 표준 흐름도, 대시보드, 테크 UI, 사이버 화면, 사실 삽화를 피합니다.
